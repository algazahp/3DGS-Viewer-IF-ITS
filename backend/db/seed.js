...
// Load .env dari backend/ agar PORT tersedia saat menentukan local URL
try {
...
const isR2 = r2Idx !== -1;
const r2Origin = isR2 ? (args[r2Idx + 1] || '').replace(/\/$/, '') : '';

let baseUrl;
let mode;
if (isLocal) {
  const port = process.env.PORT || 3000;
  baseUrl = `http://localhost:${port}/local-splats`;
  mode = `local  → file .sog di-serve via Express /local-splats (port ${port})`;
} else if (isR2 && r2Origin) {
  baseUrl = r2Origin;
  mode = `R2     → ${r2Origin}`;
} else if (isR2 && !r2Origin) {
  console.error('Error: --r2 membutuhkan URL sebagai argumen berikutnya.');
  console.error('Contoh: node db/seed.js --r2 https://pub-xxx.r2.dev');
  process.exit(1);
} else {
  baseUrl = '[R2_URL]';
...
console.log('file_url yang tersimpan:');
for (const s of scenes) console.log(`  [${s.id}] ${s.file_url}`);
